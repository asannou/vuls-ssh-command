...
sshCommand() {
  SSH_ORIGINAL_COMMAND="$1" ../ubuntu.sh
}
...
testAllow()
{
  ...
  sshCommand 'stty cols 1000; cat /etc/issue' > /dev/null 2>&1
  assertNotEquals 126 $?

  sshCommand 'stty cols 1000; lsb_release -ir' > /dev/null 2>&1
  assertNotEquals 126 $?
  ...
  sshCommand "stty cols 1000; dpkg-query -W" > /dev/null 2>&1
  assertNotEquals 126 $?

  sshCommand 'stty cols 1000; sudo -S apt-get update' > /dev/null 2>&1
  assertNotEquals 126 $?

  sshCommand 'stty cols 1000; LANGUAGE=en_US.UTF-8 apt-get upgrade --dry-run' > /dev/null 2>&1
  assertNotEquals 126 $?
}
